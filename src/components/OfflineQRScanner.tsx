...
import {
    ...
    type OfflineTicketValidationResult,
    type OfflineValidationSummary,
    type OfflineTicketRecord,
} from "@/actions/offline-scan-actions";
...
export default function OfflineQRScanner() {
    ...
    const [selectedTicketDetails, setSelectedTicketDetails] = useState<OfflineTicketRecord | null>(null);
    const [isLoadingDetails, setIsLoadingDetails] = useState(false);

    const [duplicateWarning, setDuplicateWarning] = useState<string | null>(null);
    const [validationResults, setValidationResults] = useState<OfflineTicketValidationResult[] | null>(null);
    const [validationSummary, setValidationSummary] = useState<OfflineValidationSummary | null>(null);
    ...
    const handleValidateBulk = async () => {
        ...
        const { success, results, summary, error } = await offlineValidateTicketsBulk(stagedScans);
    ...
    const handleOpenDetails = async (qr: string) => {
        ...
        const { success, data } = await offlineGetTicketDetails(qr);
        if (success && data) setSelectedTicketDetails(data);
    ...
    return (
        <div className="w-full max-w-sm mx-auto bg-white dark:bg-zinc-900 rounded-3xl shadow-xl border border-zinc-200 dark:border-zinc-800 flex flex-col mt-4">
            {/* Offline badge */}
            <div className="flex items-center justify-center gap-1.5 px-4 py-1.5 bg-amber-500 rounded-t-3xl">
                <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse"></span>
                <span className="text-[10px] font-bold text-white uppercase tracking-widest">Offline Mode — Local Redis</span>
            </div>
            ...
            {/* Camera Area */}
            <div className="w-full bg-black relative aspect-[4/3] flex items-center justify-center overflow-hidden border-b-4 border-black dark:border-zinc-800">
                {isScanning ? (
                    <Scanner
                        onScan={handleScan}
                        ...
                        <div className="flex flex-wrap gap-2.5">
                            {stagedScans.map((scan, idx) => (
                                <button
                                    ...
                                    className={`w-11 h-11 rounded-xl flex items-center justify-center font-bold text-sm transition-all shadow-sm
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-white dark:bg-zinc-900 rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl border border-zinc-200 dark:border-zinc-800 flex flex-col max-h-[90vh]">
                        <div className="p-4 border-b border-zinc-100 dark:border-zinc-800 flex justify-between items-center bg-zinc-50 dark:bg-zinc-900/50">
                            <h3 className="font-bold text-zinc-900 dark:text-zinc-100">Ticket — Local DB Record</h3>
                        ...
                        <div className="p-5 overflow-y-auto">
                            {isLoadingDetails ? (
                                <div className="flex flex-col items-center justify-center gap-3 py-10">
                                    ...
                                    <p className="text-sm font-medium text-zinc-500">Fetching from local SQLite...</p>
                                </div>
                    ...
                    ${selectedTicketDetails.valid === 1
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-zinc-50 dark:bg-zinc-800/50 p-3 rounded-xl border border-zinc-100 dark:border-zinc-800 col-span-2">
                                            <div className="grid grid-cols-2 gap-3">
                                                <div>
                                                    ...
                                                    <p className="font-bold text-zinc-900 dark:text-zinc-100 truncate">{selectedTicketDetails.name || "N/A"}</p>
                                                </div>
                                                <div>
                                                    <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-1">Phone Number</p>
                                                    <p className="font-bold text-zinc-900 dark:text-zinc-100 truncate">{selectedTicketDetails.phone || "N/A"}</p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-zinc-50 dark:bg-zinc-800/50 p-3 rounded-xl border border-zinc-100 dark:border-zinc-800">
                                            ...
                                            <p className="font-semibold text-zinc-900 dark:text-zinc-100 truncate">{selectedTicketDetails.typeid}</p>
                                        </div>
                                        <div className="bg-zinc-50 dark:bg-zinc-800/50 p-3 rounded-xl border border-zinc-100 dark:border-zinc-800">
                                            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-1">Ticket Type</p>
                                            <p className="font-semibold text-zinc-900 dark:text-zinc-100">{getTypeName(selectedTicketDetails.type || 0)}</p>
                                        </div>
                                        <div className="bg-zinc-50 dark:bg-zinc-800/50 p-3 rounded-xl border border-zinc-100 dark:border-zinc-800">
                                            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-1">Vendor</p>
                                            <p className="font-semibold text-zinc-900 dark:text-zinc-100">{getVendorName(selectedTicketDetails.vendor || 0)}</p>
                                        </div>
                                        <div className="bg-zinc-50 dark:bg-zinc-800/50 p-3 rounded-xl border border-zinc-100 dark:border-zinc-800">
                                            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-1">User ID</p>
                                            <p className="font-semibold text-zinc-900 dark:text-zinc-100 truncate">{selectedTicketDetails.userId || "N/A"}</p>
                                        </div>
                                    </div>
